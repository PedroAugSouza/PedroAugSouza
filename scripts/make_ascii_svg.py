...
a dark/blank background washes out to empty) reads as neat and legible.
...
# has the background composited to solid dark + autocontrast applied.
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "source-prepped.png")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "peu-ascii.svg")

WHOAMI = "Pedro \"PEU\" Souza"
PROMPT_USER = "peu"
...
CONTRAST = 1.05
BRIGHTNESS = 1.0
GAMMA = 0.55  # <1 brightens mids (since lum in [0,1]) -> face lands in sparser chars
SHARPEN = False
